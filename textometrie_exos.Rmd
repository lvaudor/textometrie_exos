...
```{r exo5_2-check}
grade_this({
  pass_if_equal(message="Ici la partition par la variable covid est aussi une partition par le temps... Les spécificités peuvent être aussi liées à cela plutôt qu'à la situation sanitaire!...")
  fail(hint = TRUE)
})
```
...
```{r exo5_3-check}
grade_this({
  fail_if_code_feedback(hint = TRUE)
  pass("Oui. Notez que l'évaluation pour les graphiques est minimale -le code tourne, donc ça passe-. Donc charge à vous de voir si le graphique correspond bien à l'attendu ;-).")
})
```
...
On repart des résultats du STM réalisée dans le Descriptoire (`topic_model`).
Essayez de résumer les thématiques (en vous basant sur le résultat de `summary(topic_model)` affichée dans le Descriptoire).
...
```{r exo5_5a, exercise=TRUE, exercise.lines=20,exercise.setup="part5_5-setup"}
...
grade_this({
  fail_if_code_feedback(hint = TRUE)
  pass_if_equal(message="Tout-à-fait. Cette reformulation des thématiques peut s'avérer extrêmement utile pour des analyses postérieures. ")
})
...
```{r part5_5b-setup, echo=FALSE}
library(tidyverse)
library(tidytext)
tib_meta=readr::read_csv2("data/ministereco_tib_meta.csv")
topic_model=readRDS("data/ministereco_topic_model.RDS")
```
...
```{r exo5_5b, exercise=TRUE, exercise.lines=5, exercise.setup="part5_5b-setup"}
...
```{r exo5_5b-check}
grade_this({
  pass_if_equal(message="La fonction tidy va nous permettre d'exploiter les résultats du STM relativement facilement dans le cadre du tidyverse...")
  fail(hint = TRUE)
})
```
...
```{r plot_topics_ministre, echo=TRUE}
...
```{r interpretation_ACP}
question("Cette représentation suggère que (réponse multiple, cocher les propositions vraies):",
    answer("Barbara Pompili et Nicolas Hulot sont proches en termes de thématiques abordées"),
    answer("La thématique réunion/délégation/acteurs est souvent abordée en lien avec la thématique collectivités/territoires", correct=TRUE),
    answer("Le changement climatique est souvent abordé de pair avec la biodiversité."),
    answer("Elisabeth Borne aborde souvent la question transports/mobilité."),
    answer("Barbara Pompili aborde souvent la question énergie/rénovation/logement",correct=TRUE),
    answer("Elisabeth Borne aborde souvent la question de France/Europe/International", correct=TRUE),
    incorrect="Humph,non, raté.",
    correct="Oui! Gardez à l'esprit qu'une ACP est une représentation imparfaite d'un jeu de données mais elle peut donner des pistes d'interprétation...",
    allow_retry=TRUE
)
```
